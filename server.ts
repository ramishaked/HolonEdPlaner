...
app.post("/api/ai/initiate", async (req, res) => {
  try {
    ...
    const { scores, answers, schoolName, schoolYear } = req.body;
...
ודא שהטקסט בעברית רהוטה ומקצועית ומכבדת.`;
    ...
    res.status(500).json({ error: error.message || "Failed to initiate AI diagnostic check." });
  }
});
...
שימו לב שהתגובה צריכה להיות JSON נקי לחלוטין ללא markdown wrapper.`;
    ...
    res.status(550).json({ error: error.message || "Failed to generate AI strategic roadmap." });
  }
});
