...
- **`order_index` הוא מרחב מספור אחד** לעקרונות עירוניים ובית-ספריים, כי `fetchPrinciples` ממפתח לפיו את `orderToId`/`shortTitles` (ה-id המספרי של ה-UI). לכן הטווחים מופרדים: **עירוני 1..999** (פעילים 1..N, מוסתרים 90+), **בית-ספרי 1000+**, ו-`principles_order_scope_ck` אוכף זאת ב-DB (מיגרציה `20260807100000`). בלי ההפרדה, העיקרון העירוני הבא היה מקבל את המספר של עיקרון בית-ספרי קיים — אחד היה נדרס במפה, והאבחון היה עלול להיכתב לעיקרון הלא נכון. `principlesAdmin` ממספר מחדש **רק** עקרונות עירוניים.
